...
| v0.6→v0.7 修订 | Founder 2026-08-18 复判批裁决（**第⑥条产品标准**：系统据企业事实知道用户未提及的重大经营情境时，不得擅自转向、也不得装作不知道，应并呈两套方案由用户选择；原文见 acceptance/runs/L3-判分记录-INT-20260818.md 复判节）：① B.4.1 INT-D02 允许答案族／禁止结果／人工裁判问题按第⑥条改写（**并呈双方案为合格形态**，失败现场 RUN-0013/0014）；② B.4.1 INT-D01 禁止结果「虚构…品牌定位」按①口径修订精确化（禁的是把定位当既定事实写入结论或事实库，模型基于商品参数的定位类评价属合理观点表达）；③ B.6.1 补登失败标签 INT_SITUATION_WITHHELD。**D01／D03 的允许答案族与判定不变**（分叉 C 窄口径：第⑥条只管「正要闷头开做」的场合）。其余条款不变 |
...
- 擅自选定六个特殊经营目标之一当用户意图；
- 虚构库存、销售目标、受众；**把品牌定位当作既定事实**写进结论（intent_summary）或升格为品牌事实写入事实库——口径（Founder 2026-08-18 复判批①）：系统数据库中有定位事实则**引用优先**；库中没有时，模型基于商品参数作出的定位类评价（如「符合高端基础款定位」）属**合理观点表达，可接受、不列问题项**，但必须写在「模型判断」栏并引用池内商品参数事实，不得当作品牌自己的定位主张；
...
**允许答案族：**（Founder 2026-08-18 判分批改写，统一产品标准②③⑤；**2026-08-18 复判批按第⑥条再改写**——本案例商品事实含 `product.lifecycle_stage=库存消化期`（CONFIRMED）而用户原话未提及，正落在第⑥条管辖的「系统正要闷头开做」场合，**合格形态＝并呈双方案**。表达视为完整：商品＋时间＋平台齐备，春节只作内容主题／场景）

两种模式共同要求：

- 用户原话解析出的主目标＝DAILY_CONTENT_OPERATION，**如实保留、不得降级成「没听懂」**；没有促销信息就不写促销；
- 系统已知的重大经营情境（OD-03 §五 登记的库存消化期）**必须并呈、不得吞掉**：`goal_resolution=RESOLVED_WITH_ALTERNATIVE`、`business_goal=DAILY_CONTENT_OPERATION`、`goal_candidates` 至少两个（按用户原话的春节主题常规方向 ＋ 基于库存消化期的备选方向）且每个候选写全 focus／tradeoffs／expected_outcome、`clarification_question` 请用户在两个方向间择一、`next_action=REQUEST_INPUT`（A.5.2 约束 8）；
- 备选方向的理由必须引用事实池内的情境事实（如 `FACT:product.lifecycle_stage`），不得凭空写「库存紧张」「卖不动」之类池外话；
- 显式列出 missing_context；把暂定内容标为 ASSUMPTION；降低 confidence；不把暂定人设写成事实。

快速模式：

- 非阻断缺失允许跨过继续，但缺什么必须显式说出来、逐项留假设、降置信；
- **跨过缺失 ≠ 跨过并呈义务**：快速模式不等于可以替用户把方向定了（第⑥条不分模式）。

增强模式：

- 只追问真正改变当前内容产出的高价值信息（可以为零个）；
- 人设／受众／材质凭证／促销边界等可选信息放在交付后提示补充，不作执行前障碍；
- 阻断项（OD-03 v1.2 口径）缺失时才进入 NEEDS_INPUT；资料补充后生成新 Context Snapshot。

**禁止结果：**

- 因「春节前」等节令词推断促销／清库存意图；
- 以促销边界缺失为由阻止内容制作（仅用户明示促销／清库存意图时促销边界才是阻断项）；
- **把系统已知的重大经营情境吞掉**——只写进内部判断栏、或径直按常规方案继续，用户侧零呈现（INT_SITUATION_WITHHELD；失败现场＝RUN-0013/0014，模型内部写下「该产品处于库存消化期…因此不能据此推断目标为 INVENTORY_ACTIVATION」后用户侧零呈现）；
- **擅自转向**该情境目标（未经用户选择就把 business_goal 定成 INVENTORY_ACTIVATION）；
- 并呈时退回光秃标签选择题（候选缺三要素）或不给澄清问题；
- 一次性把大量资料问题打包退回用户；
- 编造创始人背景、账号关系或品牌禁语；
- 不提示缺失却输出确定结论；
- 为单一任务索取大量无关资料；
- 快速模式绕过阻断项（按 OD-03 v1.2 的阻断口径）；
- 强制完成完整企业资料库。

**人工裁判问题：**

1. 是否把春节当作主题场景而非促销指令？
2. 是否既没有擅自转向库存目标、也没有装作不知道库存消化期？
3. 增强模式是否只问高价值信息、其余放到交付后提示？
4. 两套方案是否都写清了侧重点、优缺点与适用结果，用户能不能直接据此选？

**主要失败标签：**

INT_SITUATION_WITHHELD、INT_MISSING_CONTEXT_MISSED、INT_CONFIDENCE_OVERSTATED、INT_OVER_COLLECTION、INT_MODE_IGNORED、BD_FACT_FABRICATION。
...
| INT_SITUATION_WITHHELD | 系统据企业事实已知某个登记在册的重大经营情境（OD-03 §五）、用户原话未提及，却**既不并呈也不告知**——只在内部留痕或径直按常规方案继续（「装作不知道」）；擅自把目标转向该情境目标（「擅自转向」）同样入此标签。统一产品标准⑥的反面，Founder 2026-08-18 复判批新增；失败现场＝RUN-0013/0014 |
