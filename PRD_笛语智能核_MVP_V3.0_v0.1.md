...
| v0.1→v0.2 修订 | 2026-08-18 内容真实性三层边界批次：仅「批准」基线行随 A v0.3 / B v0.5 升版同步，其余正文零变更（基线行属正文，digest 冻结规则下改正文必须升版）。文件名沿用历史命名不改（改名牵动登记册与工具引用面），版本以本控制块为准。裁决记录见《裁决台账》08-18 块 C 行 |
| 日期 | 2026-08-16 |
