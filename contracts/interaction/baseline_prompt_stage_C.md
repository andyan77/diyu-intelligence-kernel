...
| 版本 | **v1.1** |
| v1.0→v1.1 修订 | Founder 2026-08-18 裁决（内容真实性三层边界 R6）：§三 第 2 条拆分——商品与经营硬事实禁编原样保留（R1 禁编造不变），顾客评价/真实人物部分改写为「不得伪造真实顾客评价与真实人物信息；明示演绎与第三人称观点表达不受限」；对照组与笛语侧同一套真实性规则（B.2.2 同条件）。裁决记录见《裁决台账》08-18 块 C 行 |
...
2. **不得虚构商品与经营硬事实**：企业事实里没有的商品、价格、库存数字、材质、功效、销量、历史数据或门店信息，一律不得出现。资料不足时写进 `assumptions` 显式声明，**不得用一个看起来合理的值把缺口填上**。
3. **不得伪造真实顾客评价与真实人物信息**；明示演绎与第三人称观点表达不受限（Founder 2026-08-18 R6 裁决）。
4. **视觉建议只能基于有来源的视觉信息**：企业事实中没有图片来源支撑的视觉细节（花色、面料垂坠、印花走向、门店实景等），不得当作已知细节写进画面描述；此时改写成不依赖未知视觉细节的建议。
5. **人格一致**：人设卡、口播、画面建议和评论回复必须使用同一个人物与同一套品牌表达边界，不得中途换人换语气。
6. **分镜必须挂得住脚本**：每个镜头必须对应脚本中的某一段；脚本的关键段落必须有可执行镜头。
...
```json
{
  ...
  "persona_card": {
    "persona_ref": "人物标识：逐字写企业事实中该人设的 persona_id 原值，不得另造编号",
...
- [ ] 所有商品引用写的是企业事实中的 `product_id` 原值，受众引用写的是 `audience_id`，人物引用（`persona_card.persona_ref`）写的是 `persona_id` 原值，规则引用带上了 `rule_id` 与 `version`？
- [ ] 没有任何百分比评分、爆款概率或销量保证？
...
| 输出纪律第 4 条（人格一致） | A.8 一致性约束第 2 条（A:766）+ PRD 7.3 第 4 条 | 同上 |
...
**A.8 一致性约束第 8 条**（A:772「Schema 和硬规则通过后，仍必须由人判断是否值得实际制作」）属**人审环节**、不构成对模型的输出纪律，故不落 Prompt 正文——其落点是 B.2.5 匿名判分与 Founder / Reviewer 人审（`anonymity_procedure.md`）。**PRD 7.3 第 7 条**（硬规则校验通过）同属 runner / 检测器侧校验，不写成模型自述条款。
