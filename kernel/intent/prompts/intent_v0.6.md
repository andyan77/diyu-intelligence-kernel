---
prompt_version: "v0.6"
# v0.6（2026-08-18，校准批二尾批，ATT-0012；Founder **数字表达裁决**配套）：判定纪律 6 补写法引导——
#   优先写快照原值或原值区间；确需概括只许**保守方向**（说下限往低取、说上限往高取），
#   「约/接近/大概/左右」这类无法由原值重算的形式一律不写。失败 run：RUN-0019/0020
#   （模型把四色山羊绒 9.2/9.7/9.8/9.9% 概括成「9%以上」触 A6 判红；裁决后该形式合法，
#   但需与检测器 v0.5 封闭清单同口径表达）。只动本条纪律文本，闸与检测器均不动。
...
6. **数字必须能溯源。** `intent_summary` 里出现的任何阿拉伯数字，必须逐字来自事实池或任务原话；不确定就不要写数字。
   **写法（v0.6）**：优先写事实池里的**原值**或**原值区间**（如「9.2%–9.9%」「四色分别为 9.8%／9.7%／9.2%／9.9%」）。确有必要概括时只许**保守方向**——说下限就往低取（原值最小 9.2% → 只能写「9% 以上」，不能写「10% 以上」），说上限就往高取；**不要写「约 X」「接近 X」「大概 X」「X 左右」**这类无法由原值重算的说法（系统会判为不可核实）。同一条纪律适用于 `model_judgments` 与任何其它文本字段。
