...
| 校准修订批签字 | Founder（Faye） | 2026-08-18T06:41:01+08:00（本机时钟读数；事实顺序晚于上一批，由 git 历史与台账行序背书） | 20 份 Manifest（校准级联后全量：A v0.4 指针 + B v0.6 指针 + case_version 升格 16 份 v0.5 / 4 份 v0.6）；A v0.4（七枚举 DAILY_CONTENT_OPERATION + 候选三要素）、B v0.6（INT 三案例预期族按五条统一产品标准改写 + INT_TASK_ESCALATED）、OD-03 v1.1（条件阻断）、S01 卡 ELI-0101 改判、判分器 v0.4 随本批生效；Founder 本轮批复原文「确认签字，生效」；裁决真源 acceptance/runs/L3-判分记录-INT-20260818.md | 现行有效（签字提交只动 20 行 approved_at，内容指纹由门 R6 实时核验） |
| 校准批二签字（第⑥条并呈） | Founder（Faye） | 2026-08-19T00:20:22+08:00（UTC+8 真值换算；上一批 06:41:01 系本机时钟直读，时序倒挂已在台账披露，本批起改为换算值） | 20 份 Manifest（校准批二级联后全量：A v0.5 指针 + B v0.7 指针 + case_version 升格 16 份 v0.6 / 4 份 v0.7）；A v0.5（goal_resolution 第四取值 RESOLVED_WITH_ALTERNATIVE + 约束8）、B v0.7（INT-D02 按第⑥条并呈双方案 + INT-D01 定位类评价口径精确化 + INT_SITUATION_WITHHELD）、OD-03 v1.2（§五 重大经营情境登记穷举表 + §三 移出出镜人/受众）随本批生效；Founder 本轮批复原文「确认签字，生效」；裁决真源 acceptance/runs/L3-判分记录-INT-20260818.md 复判节 | 现行有效（签字提交只动 20 行 approved_at，内容指纹由门 R6 实时核验；§四 永久规矩第六次执行） |
